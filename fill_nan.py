...
#----------------------load data--------------------------------
df_tr = []
for i,line in enumerate(open('/Users/caoyuru/Downloads/毕业论文/搜索数据用户画像/data/user_tag_query.10W.TRAIN',encoding='GB18030')):
    segs = line.split('\t')
    row = {}
    row['Id'] = segs[0]
    row['age'] = int(segs[1])
        ...
        for query in line.split('\t'):
            ...
            for gram in [1,2]:
                for i in range(len(words) - gram + 1):
                    tokens += ["_*v[_".join(words[i:i+gram])]
...
X_sp = tfv.fit_transform(df_all['query'].iloc[0:1000])
