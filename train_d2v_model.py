...
df_all = pd.read_csv('./data/' + 'all_v2.csv',encoding='utf8')
...
#-------------------train dbow doc2vec---------------------------------------------
df_lb = pd.read_csv('./data/' + 'all_v2.csv',usecols=['Education','age','gender'],nrows=200000)
ys = {}
...
for i in range(2):
    ...
    doc_list = Doc_list('alldata-id.txt')
    d2v.train(doc_list, epochs=d2v.epochs, total_examples=d2v.corpus_count)
